...
export class ProductoPage implements OnInit {
  ...
  constructor(private activiteRouter:ActivatedRoute,
      ...
      private alertController:AlertController , private _productoService:ProductoService,private storage:Storage,private router:Router, private formBuilder:FormBuilder) {
  ...
  onGuardar(){
    if (this.miFormulario && this.miFormulario.valid) {
       ...
       this._productoService.save(this._producto,this.access_token).subscribe(resp=>{
              setTimeout(() => {
                // Ocultar loading cuando la solicitud sea exitosa
                this._loading = false;
